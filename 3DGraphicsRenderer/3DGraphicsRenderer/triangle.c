...
//Function to draw a texutred pixel at postition x and y sing interpolation
...
void draw_texel(
    ...
    //Map the UV coordinate to the full texture widht and hegiht.
...
void draw_filled_triangle(int x0, int y0, int x1, int y1, int x2, int y2, uint32_t color){
    // Sorting the vertices by y component ascending.
    if (y0 > y1){
        int_swap (&y0, &y1);
        int_swap (&x0, &x1);
    }
...
void draw_textured_triangle (
    ...
    //Create vector points after we sort verticies
    ...
    if(y1 - y0 !=0){
        for (int y = y0; y <= y1; y++){
            ...
            if(x_end < x_start) {
                int_swap(&x_start, &x_end); // swap if x start is to the right of x_end
            }
